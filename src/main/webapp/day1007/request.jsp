...
<body>
<%
String[] blockIp={"192.168.10.212","192.168.10.213","192.168.10.218","192.168.10.216",
		"192.168.10.222","192.168.10.225","192.168.10.229","192.168.10.230"};
...
<ul>
	...
	외부 컴퓨터에서 접속하는 경우에는 IPv4의 주소(192.168.10.223)가 나온다. -->
	<li>접속자 Port Number: <%= request.getRemotePort() %></li>
	<li>QueryString: <%= request.getQueryString() %></li>
	<!-- QueryString은 GET방식에만 존재한다. POST방식엔 존재하지 않음. -->   
	<li>이름이 유일한 web parameter(요청 값)얻기: <%= request.getParameter("name") %></li>
	<li>이름이 중복되는 web parameter(요청 값)얻기: <%= request.getParameter("age") %></li>
	<!-- 에러는 발생하지 않고 가장 앞의 값만 얻는다. -->
	<li>이름이 중복되는 web parameter(요청 값)얻기:
	<%
		String[]ages=request.getParameterValues("age");
		//최초 요청이거나, age가 존재하지 않는 경우 배열의 null이 할당된다.
		if(ages != null){
			for(String age:ages){
				out.print(age);
				out.print(" ");
...
<a href="http://192.168.10.223/jsp_prj/day1007/request.jsp?name=kim&age=20&age=21">요청</a>   
<form method="post" action="request.jsp" enctype="application/x-www-form-urlencoded">
<!-- 자신 페이지를 다시 요청할 때에는 action속성을 생략할 수 있다. -->
<input type="hidden" value="park" name="name">
<input type="hidden" value="25" name="age">
<input type="hidden" value="26" name="age">
