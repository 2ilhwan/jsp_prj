...
<div id="wrap">
	<%-- 객체 생성 
	page: JSP페이지가 요청 될 때마다 객체 생성, 생성된 객체는 해당 JSP에서만 사용된다.
	request: JSP페이지가 요청 될 때마다 객체가 생성이 된다. / 생성된 객체는 해당 JSP와 forward로 이동한 JSP에서 사용된다.
	session: 웹 브라우저마다 하나의 객체가 생성, 생성된 객체는 모든 JSP에서 사용된다.
	application: 최초 접속자에 의해 단 하나의 객체가 생성된다. / 생성된 객체는 모든 JSP에서 사용된다. 모든 접속자는 하나의 객체를 사용한다.
	--%>
	...
	당신은 이 페이지에 <strong><jsp:getProperty property="cnt" name="cVO"/></strong>번째 방문자입니다.
